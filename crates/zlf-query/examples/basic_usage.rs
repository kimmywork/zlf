...
fn main() {
    ...
    // Create planner
    let planner = QueryPlanner::open(&test_dir).unwrap();
    ...
    let edge1 = Edge::new("knows".to_string(), alice.id.clone(), bob.id.clone(), HashMap::new());
    ...
    let edge2 = Edge::new("works_at".to_string(), bob.id.clone(), acme.id.clone(), edge_props);
    ...
    println!("Retrieved Alice: {:?}", retrieved_alice.unwrap().properties.get("name"));
    
    let retrieved_bob = planner.get_node(&bob.id).unwrap();
    assert!(retrieved_bob.is_some());
    println!("Retrieved Bob: {:?}", retrieved_bob.unwrap().properties.get("name"));
