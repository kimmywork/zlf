...
fn run_zlf_command(request: &str) -> (String, String, bool) {
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(request.as_bytes()).expect("Failed to write to stdin");
...
fn test_add_edge_command() {
    ...
    let add_edge = format!(
        ...
        db_path.display(), node1_id, node2_id
    );
...
fn test_import_and_export() {
    ...
    let import_request = format!(
        ...
        db_path.display(), import_file.display()
    );
