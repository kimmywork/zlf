...
struct AppState {
    ...
    db: RwLock<Option<Arc<ZlfDatabase>>>,
}
...
async fn ensure_db(state: &AppState, path: &str) -> Result<Arc<ZlfDatabase>, String> {
...
async fn handle_request(request: Request, state: &AppState) -> Response {
    ...
    match request {
        ...
        Request::IndexEmbedding {
            ...
            let embedding_result = if let Some(embedding) = embedding {
                Ok(embedding)
            } else if let Some(text) = text {
                let provider = create_provider(embed_config);
                provider.embed(&text).await.map_err(|e| e.to_string())
            } else {
            ...
            match embedding_result {
                Ok(embedding) => match ensure_db(&state, &path).await {
                    Ok(db) => match db.index_embedding(&node_id, &embedding, &provider_name) {
                        Ok(_) => Response::Success {
...
fn import_json(db: &ZlfDatabase, file: &str) -> Result<usize, String> {
...
fn export_json(db: &ZlfDatabase, file: Option<&str>) -> Result<serde_json::Value, String> {
