...
impl<'a> IndexedStorageFactWriter<'a> {
    ...
    pub fn apply_fact(&self, fact: &Term) -> WamResult<()> {
        ...
        index_fact_text(fact, self.vector, self.embedder)
    }
}
...
fn index_fact_text(
    ...
    match (name, args) {
        ("node", [id, props]) => index_object_text(atom(id)?, props, vector, embedder),
        ("node", [id, _, props]) => index_object_text(atom(id)?, props, vector, embedder),
        ("property", [id, _, value]) => index_value_text(atom(id)?, value, vector, embedder),
        (name, [id, value]) if name.starts_with("prop_") => {
            index_value_text(atom(id)?, value, vector, embedder)
        }
        _ => Ok(()),
    }
}
...
fn index_object_text(
    ...
    if let Term::Object(entries) = term {
        for (_, value) in entries {
            index_value_text(node_id, value, vector, embedder)?;
        }
    }
    Ok(())
}
...
fn index_value_text(
    ...
    match term {
        Term::Atom(value) | Term::String(value) => index_string(node_id, value, vector, embedder),
        Term::List(items) => index_items(node_id, items, vector, embedder),
        Term::Object(entries) => index_entries(node_id, entries, vector, embedder),
        _ => Ok(()),
    }
}
...
fn index_items(
    ...
    for item in items {
        index_value_text(node_id, item, vector, embedder)?;
    }
    Ok(())
}
