...
use super::proof::{rule_clause_with_id, stable_rule_id};
...
impl CompiledRuleArtifact {
    pub fn compile(rule: &PrologRule) -> WamResult<Self> {
        ...
        let clause_id = stable_rule_id(rule);
        let proof = rule_clause_with_id(rule, clause_id.clone());
        Ok(Self {
            clause_id,
            key,
            source: rule.clone(),
            temp_base,
            program: WamCodegen::compile_rule_clause_with_proof(rule, temp_base, proof)?,
        })
    }
...
impl<'a> StorageRuleStore<'a> {
    ...
    pub fn remove_rule(&self, rule: &PrologRule, remove_all: bool) -> WamResult<usize> {
        let key = predicate_key(&rule.head).ok_or(WamError::ExpectedFunctor(0))?;
        let mut removed = 0;
        for (storage_key, value) in self
            .storage
            .scan_prefix(&rule_prefix(&key))
            .map_err(provider_error)?
        {
            let artifact: CompiledRuleArtifact =
                bincode::deserialize(&value).map_err(provider_error)?;
            if artifact.source == *rule {
                self.storage
                    .delete_raw(&storage_key)
                    .map_err(provider_error)?;
                removed += 1;
                if !remove_all {
                    break;
    ...
    pub fn remove_rules_for(&self, key: &PredicateKey) -> WamResult<usize> {
        let entries = self
            .storage
            .scan_prefix(&rule_prefix(key))
            .map_err(provider_error)?;
