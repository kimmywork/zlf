...
use super::proof::{fact_clause, rule_clause};
...
pub fn compile_query_program_with_bindings(
    ...
    for fact in facts {
        let clause =
            WamCodegen::compile_fact_head_with_proof(fact, clause_temp_start, fact_clause(fact))?;
        clause_temp_start = next_clause_temp(clause_temp_start, &clause);
        groups.push(fact, clause)?;
    }
    for rule in rules {
        let clause =
            WamCodegen::compile_rule_clause_with_proof(rule, clause_temp_start, rule_clause(rule))?;
        clause_temp_start = next_clause_temp(clause_temp_start, &clause);
...
fn assemble_query_program(
    ...
    let program = if entries.is_empty() {
        WamProgram::new(instructions).with_entry(
            PredicateKey {
                name: "__no_clauses".to_string(),
                arity: 0,
            },
            0,
        )
